...
Decision: `GET /:code` resolves through a **cache-aside** over redis (`resolve-target.ts`): read
...
(`link-cache.ts`) so hot links stay warm. Redis is bounded by `maxmemory` + `allkeys-lru`
...
first, because the spec requires benchmarking before assuming the cache wins
(`bench/redirect-cache.ts`, 5000 warm lookups over 500 codes, pg + redis co-located on loopback):
...
redis is co-located, the gap widens. Misses are **not** negative-cached: codes are non-enumerable
